...
三个入口的预设栏都常驻设置上方：一个可选择或输入名称的下拉框，后面仅有 **保存/覆盖、删除** 两个方形图标，悬停可见说明。
点击已有选项，或用方向键选中后按Enter，立即应用预设；输入名称只用于筛选和命名，方向键浏览、Esc和失焦不应用参数。新名字直接保存，同名覆盖和删除需确认；取消不改预设或当前参数，删除也保留当前参数。
状态按当前三页与已存内容比较显示已保存或未保存；手改保留名称，重选同项可恢复，改回原值即解除未保存标记。下拉获得焦点时自动刷新共享列表，不自动载入。图标由Gradio直接渲染本地资源，不依赖工具栏脚本先初始化。
